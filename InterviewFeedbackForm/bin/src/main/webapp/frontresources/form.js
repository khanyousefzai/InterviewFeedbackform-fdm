...
var stringToHTML3 = function (str,name) {
...
function addTagSelect(n){
	...
	console.log(input);
	console.log("tagList"+n);
	let already = document.getElementById("selectTag");
	if(!set.contains(already)){
		set.appendChild(stringToHTML3(input,"insideTag"));
	}else{
		already.value = input;
	}
...
function enter(e,n){
	if (e.keyCode == 13) {
		...
		if(input != ''){
			...
			document.getElementById("tagInput"+n).value = '';
			if(!set.contains(already)){
				set.appendChild(stringToHTML2(input));
			...
			'<div class="Question_container">'+
			 ' <div class ="field_wrapper  ">'+
			  ...
			  '<div class ="field_wrapper  ">'+
			   ...
			   ' <input onfocusout="addTag('+num+')" onkeypress="return enter(event,'+num+')" type="text" class="field field_full" id="tagInput'+num+'" name="password">'+
			 ...
			 ' <div class ="field_wrapper  ">'+
			    ...
			    '<input type="text" class="field field_full qbody" id="lname" name="qbody">'+
			  '</div>'+
			'</div>'+
